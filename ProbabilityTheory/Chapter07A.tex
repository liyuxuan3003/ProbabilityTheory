\section{点估计}
设总体$X$的分布形式已知，但它的一个或多个参数未知。借助于总体$X$的一个样本赖估计总体未知参数的问题，称为\uwave{点估计问题}（Point Estimation）。点估计问题的一般提法如下。
...
\begin{Proof}
    ...
    \begin{Equation}&[2]
        L(\mu,\sigma^2)=\Prod[k=1][n]f(x_k)=\frac{1}{(\sqrt{2\pi}\sigma)^2}\exp[-\frac{1}{2\sigma^2}\Sum[k=1][n](x_k-\mu)^2]
    \end{Equation}
    或
    \begin{Equation}&[3]
