...
\begin{BoxProperty}[泊松分布的归一性]
    泊松分布满足分布律的归一性要求
    \begin{Equation}
        \Sum[k=0][\infty]\frac{\lambda^k\exp^{-k}}{k!}=1
    \end{Equation}
\end{BoxProperty}
\begin{Proof}
    运用指数函数$\e^{x}=1+x+x^2+\cdots$的性质即可证明
    \begin{Equation}*
        \Sum[k=0][\infty]\frac{\lambda^k\e^{-k}}{k!}=\e^{-\lambda}\Sum[k=0][\infty]\frac{\lambda^k}{k!}=\e^{-\lambda}\e^{\lambda}=1\qedhere
    \end{Equation}
\end{Proof}
...
\begin{BoxTheorem}[泊松定理]
    ...
    \begin{Equation}&[B]
        ...
        p_n^k(1-p_n)^{n-k}=\frac{\lambda^k\e^{-k}}{k!}
    \end{Equation}
\end{BoxTheorem}
\begin{Proof}
    ...
    \begin{Equation}*
        \Lim[n][\infty]\mqty(n\\ k)
        p_n^k(1-p_n)^{n-k}=\frac{\lambda^k\e^{-k}}{k!}\qedhere
    \end{Equation}
\end{Proof}
