...
\begin{BoxProperty}[均值在方差未知时的置信区间]
    ...
    \begin{Equation}
        \qty\Bigg[\xbar{X}\pm\frac{\sigma}{\sqrt{n}}t_{\alpha/2}(n-1)]
    \end{Equation}
    其两个单侧$1-\alpha$置信区间为
    \begin{Equation}
        \Biggl(-\infty~,~\xbar{X}+\frac{\sigma}{\sqrt{n}}t_\alpha(n-1)\Biggr]\qquad
        \Biggl[\xbar{X}-\frac{\sigma}{\sqrt{n}}t_\alpha(n-1)~,~+\infty\Biggr)        
    \end{Equation}
...
\begin{Proof}
    ...
    完全类似的
    \begin{Equation}
        P\qty{-t_{\alpha/2}(n-1)\leq\frac{\xbar{X}-\mu}{\sigma/\sqrt{n}}\leq t_{\alpha/2}(n-1)}=1-\alpha
    \end{Equation}
    由此即解得双侧$1-\alpha$的置信区间为
    \begin{Equation}
        \qty\Bigg[\xbar{X}-\frac{\sigma}{\sqrt{n}}t_{\alpha/2}(n-1)~,~\xbar{X}+\frac{\sigma}{\sqrt{n}}t_{\alpha/2}(n-1)]
    \end{Equation}
    类似的也可以讨论单侧置信区间的情况。
\end{Proof}
